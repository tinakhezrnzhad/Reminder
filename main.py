...
class reminding:
    ...
    def add(self , dateToday, dateNext, timeNext, comment ,is_done):
        try:
            self.cursor.execute("""
            INSERT INTO remind(dateToday, dateNext, timeNext, comment , is_done)
            VALUES (?, ?, ?,? ,?)
            """, (dateToday, dateNext, timeNext, comment, is_done))
# برای متن یعنی برو از اول تا اخر اون متن را داخل دیتابیس ذخیره کن 
            self.connection.commit()

            msg.showinfo(title="Success", message="یادآوری هابا موفقیت ثبت شدند")
    ...
    def search(self , id ):
        try:
            ...
            msg.showerror("error", "can not find what u look for !")
    ...
    def update(self, id, dateToday, dateNext, timeNext, comment , is_done):
        try:
            self.cursor.execute("""
            UPDATE remind
            SET id=? ,
            ...
            is_done=? 
            WHERE id=?                   
                """ , ( id, dateToday, dateNext, timeNext, comment, is_done, id)) 
            self.connection.commit()
